...
void setup() {
  ...
  String dirName ="/Users/cmdadmin/Documents/PDF/jpegs_r/";
...
//  srcs = new PImage[fnames.length];
  ...
  for (int i= 0; i<fnames.length; i++) {
    println("processing "+dirName+fnames[i]+" "+i+" of "+fnames.length+" images");
    comparisonBlockStates [i] = processImage(dirName+fnames[i]);
    JSONArray values;
    values= new JSONArray();
    for (int j=0; j<comparisonBlockStates [i].length; j++) {
      values.setBoolean(j, comparisonBlockStates [i][j]);
    }
    JSONObject anImageJson = new JSONObject();
    anImageJson.setBoolean("isLandscape", comparisonBlockStates [i][comparisonBlockStates [i].length-1]);
    anImageJson.setJSONArray("values", values);
    json.setJSONObject(fnames[i], anImageJson);


//    srcs [i]= loadImage(dirName+fnames[i]);
  }
...
boolean [] processImage(String fname) {
  ...
  //if portrait or even proportions
  else{
  ...
  //the extraspace is to hold 'isLandscape'
  imgblockStates = new boolean[1+(imgnumXBlocks*imgnumYBlocks)];
...
//  println(imgnumYBlocks);
  imgblockStates[imgblockStates.length-1] = isLandscape;
   img=null;
